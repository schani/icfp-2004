...
enum events {
	...
	EVENT_ANIMATE_FASTER,   /* Do what they say .. */
	...
	EVENT_STEP_FORWARD,    /* Navigation. All of these stop animation. */
	EVENT_STEP_BACKWARD,
	EVENT_ROUND_FORWARD,
...
extern int v_set_food (struct position *pos, int count);
...
/** Called when several steps are executed without updating each
    one individually.
  */
extern int v_remove_ant (struct position *pos);

/** Called at the beginning to set any markers. This is used when a state
    is restored from a file. */
extern int v_set_marker (struct position *pos, int team, int new_marker);
...
/** Called whenever ant moves. ant contains old position, to is new
  ...
  */
extern int v_move_ant (struct ant *ant, struct position *to, int new_direction);

/** Called when ant picks up food. Position is in struct ant. */
extern int v_ant_picks_up_food (struct ant *ant, int old_food_amount, int new_food_amount);

/** Called when ant picks up food. Position is in struct ant. */
extern int v_ant_drops_food (struct ant *ant, int old_food_amount, int new_food_amount);

/** Ant dies! It becomes food afterwards. */
extern int v_ant_dies (struct ant *ant, int old_food_amount, int new_food_amount);
...
?? review this for schani's suggestion. 
  */ 
extern int v_ant_changes_marker (struct ant *ant, int old_marker, int new_marker);
