...
struct ant {
	...
	int colony;    /* id of colony. */
	int id;        /* unique id for this ant. Also unique between colonies */
	int carries_food; /* Nonzero if ant carries food. */

	int state; /* state the ant is currently in. */
	int resting;  /* Moves to rest before I can move again. */
	int dead;  /* Nonzero if ant has RIP. */
};
...
struct execution_step {
	...
	int marker;        /* Marker set or cleared (0-5). */
	int action_had_no_side_effects; 
	...
	int new_ant_state; /* new state of ant, from trace file */
	int old_ant_state; /* old state of ant, computed in step_forward () */
};
