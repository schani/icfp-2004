...
let death_callback = ref None

let set_state_callback f = 
  callback := Some(f)

let set_state_death_callback f = 
  death_callback := Some(f)
...
let string_of_instruction = function
  | Sense (a, b, c, d) -> 
      sprintf "Sense %s %i %i %s" (string_of_sense_dir a) b c 
	(string_of_condition d)
  | PickUp (a, b) -> sprintf "PickUp %i %i" a b
  | Move (a, b) -> sprintf "Move %i %i" a b
  | Flip (a, b, c) -> sprintf "Flip %i %i %i" a b c
  | Mark (a, b) -> sprintf "Mark %i %i" a b
  | Unmark (a, b) -> sprintf "Unmark %i %i" a b
  | Drop (a) -> sprintf "Drop %i" a
  | Turn (a, b) -> sprintf "Turn %s %i" (string_of_left_or_right a) b
...
let step id =
  if ant_is_alive id then
	...
	| Move(st1, st2) ->
	    ...
	    if (rocky newp) || (some_ant_is_at newp) then begin
	      success := false;
	      set_state a st2
	    end
	    else begin
	...
	| instr -> 
	    begin
		  ...
		  set_state a st
	      | PickUp(st1, st2) ->
		  if a.carries_food || food_at(p) = 0 then begin
		    success := false;
		    set_state a st2
		  end
		  else begin
		  ...
		  set_state a st
	      | Move(_) -> (failwith "impossible state") 
	    end;
	    match !callback with 
	    | None -> ()
	    | Some(cb) -> cb a instr !success
